...
#ifndef STX_STX_HYBRID_COMPACT_BTREE_H_HEADER
#define STX_STX_HYBRID_COMPACT_BTREE_H_HEADER
...
static unsigned long long rdtsc_timer() {
  unsigned int lo,hi;
...
public:
    ...
    public:
        ...
        inline hybrid_iterator()
          : currnode(NULL), currnode_static(NULL), currslot(0), currslot_static(0), currtree(0), kless(NULL)
	{ }
	...
	inline void moveToPrev() {
	  ...
	  if (currnode_static == NULL) {
	    return (x.currnode == currnode) && (x.currslot == currslot) && (x.currtree == currtree);
          ...
          return (x.currnode == currnode) && (x.currslot == currslot) && (x.currnode_static == currnode_static) && (x.currslot_static == currslot_static) && (x.currtree == currtree);
        }
...
private:
    ...
    leaf_node* m_headleaf_static; //h

    /// Pointer to last leaf in the double linked leaf chain
    leaf_node* m_tailleaf;
    leaf_node* m_tailleaf_static; //h
...
public:
          ...
          m_root_static(NULL), m_headleaf_static(NULL), m_tailleaf_static(NULL) //h
    {
      ...
      //std::cout << "merge_ratio = " << btree_merge_ratio << "\n";
...
public:
    ...
    //huanchen
    inline iterator static_begin()
    {
        return iterator(m_headleaf_static, 0);
    }
    ...
    //huanchen
    inline iterator static_end()
    {
        return iterator(m_tailleaf_static, m_tailleaf_static ? m_tailleaf_static->slotuse : 0);
    }
...
public:
    ...
    //huanchen
    size_type count_static(const key_type& key) const
    {
        const node* n = m_root_static;
        if (!n) return 0;
    ...
    size_type count_hybrid(const key_type& key) const
    {
      return (count(key) + count_static(key));
    }
    ...
    //huanchen
    inline std::pair<iterator, iterator> equal_range_static(const key_type& key)
    {
        return std::pair<iterator, iterator>(lower_bound_static(key), upper_bound_static(key));
    }
...
public:
    ...
    inline std::pair<iterator, bool> insert2(const key_type& key, const data_type& data)
    {
      ...
      if (USE_MIN_MAX) {
	if (!m_root_static || key_less(key, min_key) || key_less(max_key, key)) {
	  return insert_start(key, data);
	}
      }
...
public:
    ...
    {
      if (USE_BLOOM_FILTER_STATIC) {
	if ((m_stats_static.itemcount == 0) || !KeyMayMatch_static(reinterpret_cast<const char*>(&iter.get_currnode()->slotkey[iter.get_currslot()]), sizeof(key_type), bloom_filter_static)) {
	  return;
	}
      }
	  ...
	  max_key = ln->slotkey[0];
        ...
        while (ln != NULL) {
          for (int slot = 0; slot < ln->slotuse; slot++) {
            ...
            ln_static->slotkey[curslot] = ln->slotkey[slot];
            ln_static->slotdata[curslot] = ln->slotdata[slot];
	    ln_static->slotuse++;

	    //bloom filter
	    if (USE_BLOOM_FILTER_STATIC) {
	      InsertToFilter_static(reinterpret_cast<const char*>(&(ln_static->slotkey[curslot])), sizeof(key_type), bloom_filter_static);
	    }
	    ...
	    if (ln_static->slotdata[slot_static] == (data_type)0) {
	      ...
	      //bloom filter
	      if (USE_BLOOM_FILTER_STATIC) {
		InsertToFilter_static(reinterpret_cast<const char*>(&(ln_new->slotkey[slot_new])), sizeof(key_type), bloom_filter_static);
	      }
...
#endif // !STX_STX_HYBRID_COMPACT_BTREE_H_HEADER
