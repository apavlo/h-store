...
, m_LoaderSettings(iTotalCustomers, iTotalCustomers, iStartFromCustomer, iCustomerCount, iScaleFactor, iDaysOfInitialTrades )
{
    ...
    this->m_generators["ZIP_CODE"] = &CGenerateAndLoad::GenerateAndLoadZipCode;
    this->m_generators["ADDRESS"] = &CGenerateAndLoad::GenerateAndLoadAddress;
    this->m_generators["STATUS_TYPE"] = &CGenerateAndLoad::GenerateAndLoadStatusType;
    this->m_generators["TAXRATE"] = &CGenerateAndLoad::GenerateAndLoadTaxrate;
    this->m_generators["ACCOUNT_PERMISSION"] = &CGenerateAndLoad::GenerateAndLoadCustomerAccountAndAccountPermission;
    this->m_generators["CUSTOMER"] = &CGenerateAndLoad::GenerateAndLoadCustomer;
    this->m_generators["EXCHANGE"] = &CGenerateAndLoad::GenerateAndLoadExchange;
    this->m_generators["SECTOR"] = &CGenerateAndLoad::GenerateAndLoadSector;
    this->m_generators["INDUSTRY"] = &CGenerateAndLoad::GenerateAndLoadIndustry;
    this->m_generators["COMPANY"] = &CGenerateAndLoad::GenerateAndLoadCompany;
    this->m_generators["COMPANY_COMPETITOR"] = &CGenerateAndLoad::GenerateAndLoadCompanyCompetitor;
    this->m_generators["SECURITY"] = &CGenerateAndLoad::GenerateAndLoadSecurity;
    this->m_generators["DAILY_MARKET"] = &CGenerateAndLoad::GenerateAndLoadDailyMarket;
    this->m_generators["FINANCIAL"] = &CGenerateAndLoad::GenerateAndLoadFinancial;
    this->m_generators["LAST_TRADE"] = &CGenerateAndLoad::GenerateAndLoadLastTrade;
    this->m_generators["NEWS_ITEM"] = &CGenerateAndLoad::GenerateAndLoadNewsItemAndNewsXRef;
    this->m_generators["NEWS_XREF"] = &CGenerateAndLoad::GenerateAndLoadNewsItemAndNewsXRef;
    this->m_generators["BROKER"] = &CGenerateAndLoad::GenerateAndLoadBroker;
    this->m_generators["CUSTOMER_ACCOUNT"] = &CGenerateAndLoad::GenerateAndLoadCustomerAccountAndAccountPermission;
    this->m_generators["CUSTOMER_TAXRATE"] = &CGenerateAndLoad::GenerateAndLoadCustomerTaxrate;
    this->m_generators["TRADE_TYPE"] = &CGenerateAndLoad::GenerateAndLoadTradeType;
    this->m_generators["TRADE"] = &CGenerateAndLoad::GenerateAndLoadTrade;
    this->m_generators["SETTLEMENT"] = &CGenerateAndLoad::GenerateAndLoadSettlement;
    this->m_generators["TRADE_HISTORY"] = &CGenerateAndLoad::GenerateAndLoadTradeHistory;
    this->m_generators["HOLDING_SUMMARY"] = &CGenerateAndLoad::GenerateAndLoadHoldingSummary;
    this->m_generators["HOLDING"] = &CGenerateAndLoad::GenerateAndLoadHolding;
    this->m_generators["HOLDING_HISTORY"] = &CGenerateAndLoad::GenerateAndLoadHoldingHistory;
    this->m_generators["WATCH_LIST"] = &CGenerateAndLoad::GenerateAndLoadWatchListAndWatchItem;
    this->m_generators["WATCH_ITEM"] = &CGenerateAndLoad::GenerateAndLoadWatchListAndWatchItem;
    this->m_generators["CASH_TRANSACTION"] = &CGenerateAndLoad::GenerateAndLoadCashTransaction;
    this->m_generators["CHARGE"] = &CGenerateAndLoad::GenerateAndLoadCharge;
    this->m_generators["COMMISSION_RATE"] = &CGenerateAndLoad::GenerateAndLoadCommissionRate;
    // this->m_generators["TRADE_REQUEST"] = &CGenerateAndLoad::GenerateAndLoadTradeRequest;
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadHolding()
{
    set<string> tables;
    tables.insert("HOLDING");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadHoldingHistory()
{
    set<string> tables;
    tables.insert("HOLDING_HISTORY");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadHoldingSummary()
{
    set<string> tables;
    tables.insert("HOLDING_SUMMARY");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadTrade()
{
    set<string> tables;
    tables.insert("TRADE");
    GenerateAndLoadHoldingAndTrade(tables);
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadTradeHistory()
{
    set<string> tables;
    tables.insert("TRADE_HISTORY");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadSettlement()
{
    set<string> tables;
    tables.insert("SETTLEMENT");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadCashTransaction()
{
    set<string> tables;
    tables.insert("CASH_TRANSACTION");
    GenerateAndLoadHoldingAndTrade(tables);
}
...
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadBroker()
{
    set<string> tables;
    tables.insert("BROKER");
    GenerateAndLoadHoldingAndTrade(tables);
}

/*
*   Generate and load HOLDING, HOLDING_HISTORY, HOLDING_SUMMARY, TRADE, TRADE_HISTORY, SETTLEMENT, CASH_TRANSACTION, BROKER table.
*
*   PARAMETERS:
*           the set of tables to actually generate.
*
*   RETURNS:
*           none.
*/
void CGenerateAndLoad::GenerateAndLoadHoldingAndTrade(set<string> targetTables)
{
    bool                                bRet;
    CTradeGen*                          pTradeGen;
    ...
    string debug("Generating ");
    string add("");
    for (set<string>::const_iterator it = targetTables.begin(); it != targetTables.end(); it++) {
        debug += add;
        debug += *it;
        add = ", ";
    } // FOR
    debug += " tables...";
    m_pOutput->OutputStart(debug);
    
    bool loadHolding = (targetTables.find("HOLDING") != targetTables.end());
    bool loadHoldingHistory = (targetTables.find("HOLDING_HISTORY") != targetTables.end());
    bool loadHoldingSummary = (targetTables.find("HOLDING_SUMMARY") != targetTables.end());
    bool loadTrade = true; // (targetTables.find("TRADE") != targetTables.end());
    bool loadTradeHistory = (targetTables.find("TRADE_HISTORY") != targetTables.end());
    bool loadSettlement = (targetTables.find("SETTLEMENT") != targetTables.end());
    bool loadCashTransaction = (targetTables.find("CASH_TRANSACTION") != targetTables.end());
    bool loadBroker = (targetTables.find("BROKER") != targetTables.end());
    ...
    do
    {
        if (loadTrade) pTradesLoad->Init();
        if (loadSettlement) pSettlementLoad->Init();
        if (loadTradeHistory) pHistoryLoad->Init();
        if (loadCashTransaction) pCashLoad->Init();
        if (loadBroker) pBrokerLoad->Init();
        if (loadHoldingHistory) pHoldingHistoryLoad->Init();
        if (loadHolding) pHoldingsLoad->Init();
        if (loadHoldingSummary) pHoldingSummaryLoad->Init();
        ...
        do
        {
            ...
            if ( loadSettlement && pTradeGen->GetSettlementRowCount() )
            {
                pSettlementLoad->WriteNextRecord(pTradeGen->GetSettlementRow());
            }

            if ( loadCashTransaction && pTradeGen->GetCashTransactionRowCount() )
            {
                pCashLoad->WriteNextRecord(pTradeGen->GetCashTransactionRow());
            }

            if ( loadHoldingHistory ) {
            ...
            if (iCnt % 10000 == 0)
            {
                ...
                if (loadSettlement) pSettlementLoad->Commit();      //commit
                if (loadTradeHistory) pHistoryLoad->Commit();         //commit
                if (loadCashTransaction) pCashLoad->Commit();
                if (loadHoldingHistory) pHoldingHistoryLoad->Commit();  //commit
                // Not loading TRADE_REQUEST table
