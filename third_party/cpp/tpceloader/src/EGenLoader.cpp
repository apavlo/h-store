...
//     cout << "[" << arg << "]: " << sp << " ";
    ...
    /*
     * Otherwise assume that it's a single table that they want to load
     */
    } else {
        string table(sp);
        transform(table.begin(), table.end(), table.begin(), (int(*)(int)) toupper);
        targetTables.insert(table);
        cout << "Single Table Load: " << table << endl;
        continue;
    }
    *sp = (char)tolower( *sp );
...
bool ValidateParameters()
{
    ...
    //
    INT64 temp = (INT64)(HoursPerWorkDay * SecondsPerHour * iLoadUnitSize / iScaleFactor);
    ...
    try {
        ...
        if (!targetTables.empty()) {
            for (set<string>::const_iterator it = targetTables.begin(); it != targetTables.end(); it++) {
                void(CGenerateAndLoad::* func_ptr)() = NULL;
                func_ptr = pGenerateAndLoad->getGenerateFunction(*it);
                if (func_ptr != NULL) {
                    // cout << "Calling generate method for: " << *it << endl;
                    (pGenerateAndLoad->*func_ptr)();
                } else {
                    cerr << "ERROR: No generator available for table " << *it << endl;
                    exit( ERROR_BAD_OPTION );
                }
            } // FOR
