...
public:
    ...
    void CheckValid(void) {
        ...
        DRIVERPARAM_CHECK_LE(    "iCustomerCount",           cur.iCustomerCount + cur.iStartingCustomer - 1, cur.iConfiguredCustomerCount);
    }
...
class CDriverCEPartitionSettings : public CParametersWithDefaults< struct TDriverCEPartitionSettings, struct TDriverCEPartitionSettingsState >
{
public:
    ...
    // In thise case we set the current values to 0 to indicate that they are unused.
    ...
    void CheckValid(void) {
        ...
        if ( cur.iMyStartingCustomerId == 0 && cur.iMyCustomerCount == 0 && cur.iPartitionPercent == 0 ) {
            ...
            // Spec clause 6.4.3.1 has many requirements, these are the ones that we validate here:
