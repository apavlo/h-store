    ...
    */
    void GenerateNextWL_ID()
    {
        ...
        m_row.m_watch_list.WL_ID =
            ...
            % iDefaultLoadUnitSize + iDefaultStartFromCustomer;
    }
...
public:
    CWatchListsAndItemsTable(CInputFiles inputFiles,
                            TIdent iCustomerCount,
                            TIdent iStartFromCustomer)
        : TableTemplate<WATCH_LIST_AND_ITEM_ROW>()
        , m_iRowsToGenForWL(0)
        , m_iRowsGeneratedForWL(0)
        , m_cust(inputFiles, iCustomerCount, iStartFromCustomer)
        , m_iWICount(0)
        , m_iMinSecIdx(iMinSecIdx)
        ...
        , m_bInitNextLoadUnit(false)
    {
        ...
        do
        {
            ...
            {
                m_bInitNextLoadUnit = true; // delay calling InitNextLoadUnit until the start of the row generation
            }
    ...
    */
    bool GenerateNextRecord()
    {
        ...
        for (i=0, pos=m_set.begin(); pos!=m_set.end(); ++i, ++pos)
        {
            ...
            m_SecurityFile->CreateSymbol( *pos, m_row.m_watch_items[i].WI_S_SYMB, static_cast<int>(sizeof(m_row.m_watch_items[i].WI_S_SYMB)));
        }

        bRet = false;   // initialize for the case of currently processing the last customer
    ...
    PWATCH_LIST_ROW GetWLRow() {return &m_row.m_watch_list;}
    PWATCH_ITEM_ROW GetWIRow(UINT i)
    {
        if (i<m_iWICount)
