...
#ifndef STX_STX_HYBRID_COMPACT_MULTI_BTREE_H_HEADER
#define STX_STX_HYBRID_COMPACT_MULTI_BTREE_H_HEADER
...
namespace stx_hybrid_compact_multi {
...
private:
    ...
    struct leaf_multidata_node : public node
    {
        /// Define an related allocator for the leaf_node structs.
        typedef typename _Alloc::template rebind<leaf_multidata_node>::other alloc_type;

        /// Double linked list pointers to traverse the leaves
        leaf_multidata_node * prevleaf;

        /// Double linked list pointers to traverse the leaves
        leaf_multidata_node * nextleaf;

        /// Keys of children or data pointers
        key_type  slotkey[leafslotmax];

        /// Array of data
	unsigned short slotdata_count[leafslotmax];
        data_type* slotdata[leafslotmax];

        /// Set variables to initial values
        inline leaf_multidata_node()
            : node(0), prevleaf(NULL), nextleaf(NULL)
        { }

        /// Construction during restore from node top
        inline leaf_multidata_node(const node& top)
            : node(top.level, top.slotuse), prevleaf(NULL), nextleaf(NULL)
        { }
...
public:
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        typename btree::leaf_node * currnode;
        typename btree::leaf_multidata_node * currnode_static;

        /// Current key/data slot referenced
        short currslot;
        short currslot_static;

	unsigned short currdata_pos_static;
    ...
    public:
        ...
        inline hybrid_iterator()
          : currnode(NULL), currnode_static(NULL), currslot(0), currslot_static(0), currdata_pos_static(0), currtree(0), kless(NULL)
        { }

        inline hybrid_iterator(key_compare kl)
          : currnode(NULL), currnode_static(NULL), currslot(0), currslot_static(0), currdata_pos_static(0), currtree(0), kless(kl)
        { }

        /// Initializing-Constructor of a mutable iterator
        inline hybrid_iterator(typename btree::leaf_node* l, short s, typename btree::leaf_multidata_node* l_static, short s_static, unsigned short ctree, key_compare kl)
          : currnode(l), currslot(s), currnode_static(l_static), currslot_static(s_static), currdata_pos_static(0), currtree(ctree), kless(kl)
        { }

        inline hybrid_iterator(typename btree::leaf_node* l, short s, typename btree::leaf_multidata_node* l_static, short s_static, unsigned short dpos, unsigned short ctree, key_compare kl)
          : currnode(l), currslot(s), currnode_static(l_static), currslot_static(s_static), currdata_pos_static(dpos), currtree(ctree), kless(kl)
        { }
        ...
        inline leaf_multidata_node *get_currnode_static() {
        ...
        inline unsigned short get_currdata_pos_static() {
          return currdata_pos_static;
        }
	...
	inline void print() {
	  ...
	  if (currnode_static != NULL) {
	    std::cout << "currnode_static slotuse = " << currnode_static->slotuse << "\n";
	    std::cout << "currslot_static = " << currslot_static << "\n";
	    std::cout << "currdata_pos_static = " << currdata_pos_static << "\n";
	  }
	...
	inline bool isEnd() const {
	  ...
	  return (currnode->nextleaf == NULL) && (currslot == currnode->slotuse) && (currnode_static->nextleaf == NULL) && (currslot_static == currnode_static->slotuse);
        ...
        {
          if (currtree == 1) {
            return currnode_static->slotdata[currslot_static][currdata_pos_static];
          }
	...
	inline void moveToNext() {
	  if (isBegin()) {
	    ...
	    do {
	      if (currdata_pos_static < currnode_static->slotdata_count[currslot_static] - 1) {
		++currdata_pos_static;
	      }
	      else {
		...
		currdata_pos_static = 0;
	      }
	    } while((currslot_static != currnode_static->slotuse) && (currnode_static->slotdata[currslot_static][currdata_pos_static] == (data_type)0));
          }
	...
	inline void moveToPrev() {
	  if (isEnd()) {
	    ...
	    currslot_static--;
	    if (currnode_static != NULL) {
	      currdata_pos_static = currnode_static->slotdata_count[currslot_static] - 1;
	    }
	  }
          else if (currtree == 0){
            if (currslot > 0) {
	    ...
	    } while((currslot_static != -1) && (currnode_static->slotdata[currslot_static][currdata_pos_static] == (data_type)0));
          }
	  ...
	  do {
	    moveToNext();
	  } while((currnode_static != NULL) && (currslot_static != currnode_static->slotuse) && (currnode_static->slotdata[currslot_static][currdata_pos_static] == (data_type)0));
	  ...
	  do {
	    moveToPrev();
	  } while((currnode_static != NULL) && (currslot_static != -1) && (currnode_static->slotdata[currslot_static][currdata_pos_static] == (data_type)0));	  
	  ...
	  if (isEnd() && x.isEnd())
	    return true;
	  if (currnode == NULL) {
	    return (x.currnode_static == currnode_static) && (x.currslot_static == currslot_static) && (x.currdata_pos_static == currdata_pos_static) && (x.currtree == currtree);
	  }
	  ...
	  if (currnode_static == NULL) {
	    return (x.currnode == currnode) && (x.currslot == currslot) && (x.currtree == currtree);
          ...
          return (x.currnode == currnode) && (x.currslot == currslot) && (x.currnode_static == currnode_static) && (x.currslot_static == currslot_static) && (x.currdata_pos_static == currdata_pos_static) && (x.currtree == currtree);
        }
...
private:
    ...
    node* m_root;
    node* m_root_static; //h

    /// Pointer to first leaf in the double linked leaf chain
    leaf_node* m_headleaf;
    leaf_multidata_node* m_headleaf_static; //h

    /// Pointer to last leaf in the double linked leaf chain
    leaf_node* m_tailleaf;
    leaf_multidata_node* m_tailleaf_static; //h
...
private:
    ...
    typename leaf_multidata_node::alloc_type leaf_multidata_node_allocator()
    {
        return typename leaf_multidata_node::alloc_type(m_allocator);
    }
    ...
    inline leaf_multidata_node * allocate_leaf_static()
    {
        leaf_multidata_node* n = new (leaf_multidata_node_allocator().allocate(1))
                       leaf_multidata_node();
    ...
    {
        if (n->isleafnode()) {
            leaf_multidata_node* ln = static_cast<leaf_multidata_node*>(n);
	    for (int i = 0; i < ln->slotuse; i++) {
	      free(ln->slotdata[i]);
	    }
            typename leaf_multidata_node::alloc_type a(leaf_multidata_node_allocator());
            a.destroy(ln);
            a.deallocate(ln, 1);
            m_stats_static.leaves--;
        }
...
private:
    ...
    //huanchen
    void clear_inner_recursive(node* n)
    {
        if (n->isleafnode())
        {
            leaf_node* leafnode = static_cast<leaf_node*>(n);
        ...
        else
        {
            ...
            for (unsigned short slot = 0; slot < innernode->slotuse + 1; ++slot)
            {
                if (innernode->childid[slot]->level > 1) {
                  clear_inner_recursive(innernode->childid[slot]);
                  free_node(innernode->childid[slot]);
                }
...
public:
    ...
    {
        return m_stats_static.itemcount;
    }
    ...
    //huanchen
    inline bool static_empty() const
    {
        return (static_size() == size_type(0));
    }
...
public:
    ...
    bool exists_static(const key_type& key) const
    {
        ...
        const leaf_multidata_node* leaf = static_cast<const leaf_multidata_node*>(n);

        int slot = find_lower(leaf, key);

	bool exist = (slot < leaf->slotuse) && key_equal(key, leaf->slotkey[slot]);
	if (!exist) return false;
	for (int i = 0; i < leaf->slotdata_count[slot]; i++) {
	  if (leaf->slotdata[slot][i] != (data_type)0)
	    return true;
	}
	return false;
    }
    ...
    hybrid_iterator find_static(const key_type& key)
    {
        ...
        leaf_multidata_node* leaf = static_cast<leaf_multidata_node*>(n);

        int slot = find_lower(leaf, key);
	bool exist = (slot < leaf->slotuse) && key_equal(key, leaf->slotkey[slot]);
	if (!exist) return hybrid_end();
	for (int i = 0; i < leaf->slotdata_count[slot]; i++) {
	  if (leaf->slotdata[slot][i] != (data_type)0)
	    return hybrid_iterator(NULL, 0, leaf, slot, i, 1, m_key_less);
	}
	return hybrid_end();
    }
    ...
    //huanchen
    hybrid_iterator lower_bound_static(const key_type& key)
    {
        node* n = m_root_static;
        if (!n) return hybrid_end();
        ...
        int slot = find_lower(leaf, key);
	hybrid_iterator iter = hybrid_iterator(NULL, 0, leaf, slot, 1, m_key_less);
	if (!iter.isEnd()) {
	  if (leaf->slotdata[slot][0] == (data_type)0)
	    iter++;
	}
	return iter;
    }
    ...
    hybrid_iterator lower_bound_hybrid(const key_type& key)
    {
      ...
      hybrid_iterator iter_static = lower_bound_static(key);
      ...
      else if (key_lessequal(iter.get_currnode()->slotkey[iter.get_currslot()], iter_static.get_currnode_static()->slotkey[iter_static.get_currslot_static()])) {
        return hybrid_iterator(iter.get_currnode(), iter.get_currslot(), iter_static.get_currnode_static(), iter_static.get_currslot_static(), iter_static.get_currdata_pos_static(), 0, m_key_less);
      }
      else {
        return hybrid_iterator(iter.get_currnode(), iter.get_currslot(), iter_static.get_currnode_static(), iter_static.get_currslot_static(), iter_static.get_currdata_pos_static(), 1, m_key_less);
      }
    }
    ...
    //huanchen
    hybrid_iterator upper_bound_static(const key_type& key)
    {
        node* n = m_root_static;
        if (!n) return hybrid_end();
    ...
    hybrid_iterator upper_bound_hybrid(const key_type& key)
    {
      ...
      hybrid_iterator iter_static = upper_bound_static(key);
	...
	if ((m_stats.itemcount == 0) || !KeyMayMatch(reinterpret_cast<const char*>(&key), sizeof(key_type), bloom_filter)) {
	  ...
	  return std::pair<hybrid_iterator, hybrid_iterator>(hybrid_iterator(NULL, 0, iter_lower.get_currnode_static(), iter_lower.get_currslot_static(), iter_lower.get_currdata_pos_static(), 1, m_key_less), hybrid_iterator(NULL, 0, iter_upper.get_currnode_static(), iter_upper.get_currslot_static(), iter_upper.get_currdata_pos_static(), 1, m_key_less));
	}
      }
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
      ...
      //bloom filter
      if (USE_BLOOM_FILTER)
	InsertToFilter(reinterpret_cast<const char*>(&key), sizeof(key_type), bloom_filter);
...
public:
	...
	return false;
      ...
      iter.get_currnode_static()->slotdata[iter.get_currslot_static()][0] = (data_type)0;
      return true;
    }
    ...
    {
      if (iter != hybrid_end()) {
	iter.get_currnode_static()->slotdata[iter.get_currslot_static()][iter.get_currdata_pos_static()] = (data_type)0;
	...
	erase_static(hybrid_iterator(NULL, 0, iter.get_currnode_static(), iter.get_currslot_static(), iter.get_currdata_pos_static(), 1, m_key_less));
      }
    }
...
public:
    void merge() {
      ...
      int node_count = 1; //node count per level
      int inner_level = 1;
      inner_node *in_static;
      ...
      if (m_root_static == NULL) {
	    ...
	    if (!key_equal(currkey, ln->slotkey[slot])) {
	      ...
	      currkey = ln->slotkey[slot];
	      data_count = 0;
	    }
	    data_buf[data_count] = ln->slotdata[slot];
	    data_count++;
	    m_stats_static.itemcount++;
          } //END for
          leaf_node *next_ln = ln->nextleaf;
          free_node(ln);
          ln = next_ln;
        } //END while
	...
	curslot++;
        ...
        leaf_multidata_node *ln_new = allocate_leaf_static();
	...
	if (key_lessequal(ln->slotkey[0], ln_static->slotkey[0]))
	  currkey = ln->slotkey[0];
	else
	  currkey = ln_static->slotkey[0];

        while ((ln != NULL) && (ln_static != NULL)) {
		...
		currkey = ln_static->slotkey[slot_static];
		data_count = 0;
	      }
	      for (int i = 0; i< ln_static->slotdata_count[slot_static]; i++) {
		if (ln_static->slotdata[slot_static][i] != (data_type)0) {
		  data_buf[data_count] = ln_static->slotdata[slot_static][i];
		  data_count++;
		  m_stats_static.itemcount++;
		}
	      }
          ...
          else if (slot_static >= ln_static->slotuse) {
            leaf_multidata_node *ln_static_temp = ln_static->nextleaf;
...
#endif // !STX_STX_HYBRID_COMPACT_MULTI_BTREE_H_HEADER
