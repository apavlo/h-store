    ...
    for DB in 'NVM' 'BERKELEY'; do
        for skew in 0.8 1.01 1.1 1.2; do
            for round in 1 ; do
                ...
                OUTPUT_PREFIX="$OUTPUT_DIR/ycsb-single/$round-ycsb1G-$block-$DB-T500-S$skew"
                LOG_PREFIX="/home/michaelg/h-store/logs/test/ycsb-single/$round-ycsb1G-$block-$DB-T500-S$skew"
                echo $OUTPUT_PREFIX
