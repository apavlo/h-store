    ...
    for db in 'NVM' 'BERKELEY'; do
        for skew in 0.8 1.01 1.1 1.2; do
            for round in 1 2 3 4 5 6; do
                ...
                OUTPUT_PREFIX="$OUTPUT_DIR/ycsb-T500-NoLoop-full-block/$round-ycsb1G-$block-$db-T500-S$skew"
                LOG_PREFIX="logs/test/ycsb-T500-NoLoop-full-block/$round-ycsb1G-$block-$db-T500-S$skew"
                echo $OUTPUT_PREFIX
...
#    "-Dsite.anticache_evict_size=${ANTICACHE_EVICT_SIZE}" \
                        ...
                        "-Dclient.anticache_db_blocks=$blocking" \
                        "-Dclient.anticache_dbtype=$db" \
