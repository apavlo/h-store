...
BASE_PROJECT="ycsb"
BASE_DIR=`pwd`
OUTPUT_DIR="/home/user/giardino/data-hstore/ycsb"
#BLK_CON=1
BLK_EVICT=200
AC_THRESH=200
SCALE=4
    ...
    for DB in 'NVM' ; do
        for skew in 0.8 1.01 1.25 4 8; do
            for round in 1; do
                ...
                LOG_PREFIX="/home/user/giardino/h-store/logs/ycsb-nvn/$round-ycsb1G-$block-$DB-S$skew-${PARTITIONS}p-${BLK_CON}c-${N_HOSTS}h-${CLIENT_THREADS_PER_HOST}ct-sc$SCALE-${BLOCK_SIZE_KB}kb-${BLK_EVICT}b-${AC_THRESH}th-${DURATION_S}s-${block_merge}"
                echo "log = $LOG_PREFIX"
                echo $OUTPUT_PREFIX
