...
OUTPUT_DIR="/home/michaelg/data-hstore/ycsb"
    ...
    for DB in 'NVM' 'BERKELEY'; do
        for skew in 0.8 1.01 1.1 1.2; do
            for round in 1 ; do
                OUTPUT_PREFIX="$OUTPUT_DIR/ycsb-T500-NoLoop-full-block/$round-baseline-ycsb1G-$DB-T500-S$skew"
                LOG_PREFIX="logs/test/ycsb-T500-NoLoop-full-block/$round-baseline-ycsb1G-$DB-T500-S$skew"
                echo $OUTPUT_PREFIX
