...
public abstract class DistributedJoin {
    ...
    /*
    static {
        Stack<Integer> dep_ids = new Stack<Integer>();
        ...
        { 
            ...
            node0.setDependencyId(dep_id++);
            dep_ids.push(node0.getDependencyId());
            
            ProjectionPlanNode node1 = new ProjectionPlanNode(node_id++);
            node0.getChildren().add(node1);
            node1.getParents().add(node0);
        ...
        // SeqScans
        //
        for (int ctr = 0; ctr < num_of_scans; ctr++) {
            SendPlanNode node0 = new SendPlanNode(node_id++);
            node0.setDependencyId(dep_id++);
            dep_ids.push(node0.getDependencyId());
        ...
        //
        {
            ...
            UnionPlanNode node1 = new UnionPlanNode(node_id++);
            for (int ctr = 0; ctr < num_of_scans; ctr++) {
                ReceivePlanNode node2 = new ReceivePlanNode(node_id++);
                node2.setDependencyId(dep_ids.pop());
                node1.getChildren().add(node2);
                node2.getParents().add(node1);
            } // FOR
            ...
            NestLoopPlanNode node3 = new NestLoopPlanNode(node_id++);
            node3.getChildren().add(node1);
            node3.getChildren().add(node2);
            node0.getChildren().add(node3);

            node1.getParents().add(node3);
