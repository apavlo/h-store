...
public abstract class SkewFactorUtil {
    ...
    /**
     * Calculate the skew factor of a histogram. This is an estimation of how uniformly
    ...
    public static double calculateSkew(int num_partitions, long total_ctr, Histogram<Integer> h) {
        ...
        if (debug) LOG.debug("Calcuating skew for histogram [num_partitions=" + num_partitions + ", total_ctr=" + total_ctr + "]");
        
        // The skew factor is a calculation of how evenly the partitions are accessed 
        double skew = 0.0d;
        
        // The best skew is when all of the partitions have an equal number of elements
        ...
        // The worst skew is always zero. This would occur if all of the counted elements
        ...
        for (int i = 0; i < num_partitions; i++) {
            ...
            skew += log;
            if (debug) sb.append(String.format(DEBUG_F, i, current, orig_ratio, ratio, log, skew));
        } // FOR
        ...
        if (debug) {
            ...
            // We use the min and max skew values to normalize the calculated skew to be between [0, 1]
            if (skew > worst) {
                System.err.println("\n" + sb.toString());
