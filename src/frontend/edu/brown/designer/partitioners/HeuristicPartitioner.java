...
public class HeuristicPartitioner extends AbstractPartitioner {
    ...
    protected class VertexComparator implements Comparator<Vertex> {
        ...
        public int compare(Vertex v0, Vertex v1) {
            Collection<Edge> edges0 = this.graph.getIncidentEdges(v0);
            Collection<Edge> edges1 = this.graph.getIncidentEdges(v1);
            int weight0 = (edges0 != null ? edges0.size() : 0);
            int weight1 = (edges1 != null ? edges1.size() : 0);
            int diff = weight1 - weight0;
            if (diff == 0) diff = v0.getTable().getGuid() - v1.getTable().getGuid();  
            return (diff);
        }
        */
        ...
        public int compare(Vertex v0, Vertex v1) {
            ...
            if (diff == 0) diff = v0.getCatalogItem().compareTo(v1.getCatalogItem());
            return (diff);
        }
    } // END CLASS
    ...
    public PartitionPlan generate(final DesignerHints hints) throws Exception {
        ...
        // ----------------------------------------------------
        // (1) Fork a new thread to generate a partition tree for each stored procedure
        // ----------------------------------------------------
    ...
    protected PartitionTree generatePartitionTree(Procedure catalog_proc, AccessGraph agraph, DesignerHints hints) throws Exception {
        ...
        // We take multiple passes through the partition trees until we come up with one
        ...
        while (true) {
            ...
            //
            // We add the procedure that was used to generate this ptree for debugging
            ...
            //
            // Now go through and see if there any tables that need to be replicated
            //
            ...
            //
            // If there are no tables that are conflicted, then there is nothing else we can do
            //
            if (replication_check.getConflictVertices().isEmpty()) break;
            
            //
            // Examine the edges that we created and see if there is a common ancestor that the 
            ...
            for (Vertex conflict_vertex : replication_check.getConflictVertices()) {
                ...
                //
                // If we have a column that is used in both trees, then that's the one we'll want to partition
                // our buddy on...
                //
                boolean valid = true;
            ...
            if (forced_dependency) continue;
            
            //
            // Now for each candidate we need to check whether replicating would actually improve
            // the performance of this procedure. So first we need to get a baseline cost
            ...
            // If this is the first time we are doing this for this procedure, then add in a blank
            // replication set so that we can get the baseline cost
            //
            Set<Table> best_set = null;
            double best_cost = Double.MAX_VALUE;
            if (best_cost == overall_best_cost) candidate_sets.add(new HashSet<Table>());
            for (Set<Table> replication_set : candidate_sets) {
                ...
                for (Table catalog_tbl : proc_tables) {
                    ...
                    //
                    // Otherwise partition it according to the current partition tree
                    //
                    } else {
                ...
                if (cost <= best_cost) {
                    ...
                    //
                    if (best_set != null && cost == best_cost && best_set.size() < replication_set.size()) continue;
            ...
            // If the overall_best_cost is equal to the best cost, then we can go
            ...
            //
            // If we're down here, then there is nothing else that needs to be done!
            //
            break;
        } // WHILE
        return (ptree);
    ...
    protected void buildPartitionTree(final PartitionTree ptree, final Vertex parent, final AccessGraph agraph, final DesignerHints hints) throws Exception {
        ...
        // Replication: Check whether the hints tell us to force this table as replicated
        ...
        for (Vertex vertex : agraph.getSuccessors(parent)) {
            ...
            if (hints.force_dependency.containsKey(CatalogKey.createKey(vertex_tbl))) {
                ...
                // We'll force this  We're allowed to break this forced dependency if the current parent
                // is a descendant of the table that the child should be forced to
            ...
            // If this vertex is already in the PartitionTree, we need to check whether our 
            // current parent has an edge with a greater weight to the vertex than the one it
            // currently has in the tree.
            //
            // What if the other vertex is a direct ascendant of the current parent? That means
            // if we break the edge then the whole path will get messed up and all the vertices
            // will be orphans again. Therefore, I think it should only move the vertex if 
            // the edge is greater *AND* it's not an ascendant of the current parent.
            if (ptree.containsVertex(vertex)) {
                ...
                if (vertex_orig_parent != null) {
                    ...
                    try {
                        ...
                        // TODO: Need to think about whether it makes sense to take the total weight
                    ...
                    //
                    // If the edge has changed, then we need to add it to our list of next vertices to visit.
                    // What if there isn't an AttributeSet that matches up with the parent? Then
                    ...
                    if (!force && max_edge.equals(orig_edge)) {
                        ...
                        //
                        // Check whether this child was our parent before, then we need to make sure that
                        // we switch ourselves to the HASH partition method instead of MAP
                        //
        ...
        if (!next.isEmpty()) {
            ...
            //
            // If we're the root, then we need to select our partitioning attribute
            //
            if (is_root) {
                ...
                for (Vertex child : next) {              
                    ...
                    for (Edge edge : agraph.findEdgeSet(parent, child)) {
                        ...
                        if (!AccessGraph.AccessType.JOINS.contains(type)) continue;
                ...
                if (attributes.size() > 1) attributes.generateSubSets();
                if (debug) {
                    LOG.debug(attributes.debug());
                    LOG.debug("---------------------");
                }
                ...
                //
                // Now get the list of AttributeSets that have the highest weights
                //
                ...
                if (asets.isEmpty()) {
                    ...
                    return; //throw new Exception("ERROR: Failed to generate AttributeSets for parent '" + parent + "'");
                } else if (asets.size() > 1) {
                    //
                    // XXX: Pick the attribute with the longest path to a root in the dependency graph
                    //
                ...
                for (Edge edge : agraph.findEdgeSet(parent, child)) {
                    ...
                    if (!entries.isEmpty()) {
                        ...
                        try {
                            ...
                            //
                            // For now we can only link ourselves to the parent's attribute
                            //
                            child.setAttribute(ptree,
                                               PartitionTree.VertexAttributes.PARENT_ATTRIBUTE.name(),
                                               parent.getAttribute(ptree, PartitionTree.VertexAttributes.ATTRIBUTE.name()));
                            
//                            if (parent.getTable().getName().equals("DISTRICT")) {
//                                System.out.println(parent.getTable().getName() + "." + parent_attribute.getName() + " -> " + child.getTable().getName() + "." + CollectionUtil.getFirst(entries).getName());
//                                System.exit(1);
//                            }
                        } catch (Exception ex) {
        ...
        // we need to decide how to partition it based on the self-referencing edges
        //
                ...
                new VertexTreeWalker<Vertex, Edge>(ptree) {
                    ...
                    protected void callback(Vertex element) {
                        ...
                        if (method == PartitionMethodType.REPLICATION) {
                            ...
                            //
                            // If this vertex is a dependent on a parent, then we also need to get the
                            // mapping of columns
                            //
                            if (parent != null) {
                                ...
                                method = PartitionMethodType.MAP; // Why do we have to set this here?
//                                if (parent_attribute == null) {
//                                    PartitionEntry entry = new PartitionEntry(element.getTable(), method, attribute, parent_table, parent_attribute);
...
//                                }
        ...
        //
        // Now for each relation, make a tally for the different ways that it could be partitioned
        // This will then be used to generate the final PartitionMapping
        //
        PartitionPlan pplan = new PartitionPlan();
        for (Table catalog_tbl : info.catalog_db.getTables()) {
            //
            // For each table, look at the PartitionPlan entries that we created above and see
            ...
            for (PartitionTree ptree : partition_plans.keySet()) {
                ...
                //
                // We found a partition plan that references our table, so then we need to grab
                // the entry and include it in our count list. Note that the PartitionPlan.Entry
                // object knows how to properly tell whether it has the same attributes as 
                ...
                if (partition_plan.getTableEntries().containsKey(catalog_tbl)) {
                    ...
                    if (entry.getMethod() == PartitionMethodType.HASH && entry.getAttribute() == null) {
                        ...
                        //
                        // We need to weight this entry by the weight of the PartitionTree
                        // that it was derived from
                        //
                        double count = ptree.getWeight();
                        if (counts.containsKey(entry)) count += counts.get(entry);
            ...
            while (it.hasNext()) {
                ...
                for (PartitionEntry entry1 : counts.keySet()) {
                    ...
                    if (entry0.getMethod() == PartitionMethodType.HASH &&
                        ...
                        entry0.getAttribute().equals(entry1.getAttribute())) {
                        remove = true;
                    }
                } // FOR
                if (remove) {
            ...
            //
            // If our counts for the current table is not empty, then we need to need to pick
            ...
            if (!counts.isEmpty()) {
                ...
                for (PartitionEntry entry : counts.keySet()) {
                    ...
                    // add it to our list of possible selections. Note that if it's greater than the 
                    // current max weight, then we need to clear our previous entries
                    // 
                    if (entry_cnt >= max_cnt) {
                        if (entry_cnt > max_cnt) picked_entries.clear();
                        ...
                        for (Vertex descendent : descedents) {
                             if (descendent != element && agraph.containsVertex(descendent)) {
                                 ...
                                 for (Edge edge : agraph.findEdgeSet(element, descendent)) {
                                     ...
                                     // long paths more important than short ones, because that means we will
                                     // get to include more related partitions together
