...
//        }
        ...
        if (this.upper_bounds_pplan == null) {
            ...
            cost_model.clear();
            double cost = cost_model.estimateWorkloadCost(this.info.catalog_db, this.info.workload);
    ...
    protected class TraverseThread extends Thread {
        ...
        private void initializeProcedures() {
            ...
            for (Procedure catalog_proc : this.info.catalog_db.getProcedures()) {
                ...
                if (this.search_procs.contains(catalog_proc)) {
                    ...
                    if (trace.get()) LOG.trace("Updating ConstraintPropagator for fixed " + catalog_proc);
        ...
        protected void traverse(final StateVertex parent, final int idx) throws Exception {
            ...
            // Just make sure that if we have a VerticalPartitionColumn in our list of
            ...
            for (CatalogType attribute : current_attributes) {
                ...
                if (is_table) {
                    ...
                    // VerticalPartitionColumn
                        ...
                        assert(CatalogUtil.getDatabase(vp_col).equals(info.catalog_db)) : String.format("VP_COL[%d] != INFO[%d]", CatalogUtil.getDatabase(vp_col).hashCode(), info.catalog_db.hashCode());
                    ...
                    try {
                        memory = this.memory_estimator.estimate(info.catalog_db, info.getNumPartitions(), tablesToEstimate);
                    } catch (Throwable ex) {
...
//                this.graph.addEdge(new StateEdge(), parent, state, EdgeType.DIRECTED);
                ...
                if (!parent.isStartVertex()) {
                    ...
                    if (!is_valid) { //  && debug.get()) {
                        ...
                        cost_model.clear();
                        double cost2 = this.cost_model.estimateWorkloadCost(info.catalog_db, info.workload, filter, best_vertex.cost);
                ...
                if (complete_solution &&
                    ...
                    if (debug.get()) {
                        ...
                        if (this.cost_model.hasDebugMessages()) LOG.debug("Last Cost Model Info:\n " + this.cost_model.getLastDebugMessage());
                    }
...
//                            LinkedList<String> attributes = AbstractPartitioner.generateProcParameterOrder(info, this.info.catalog_db, catalog_proc, hints);
//                            
...
//                    }
            ...
            if (!is_table && !this.halt_search) {
                ...
                ProcParameter current_param = CatalogKey.getFromKey(info.catalog_db, best_key, ProcParameter.class);
