...
public class ConstraintPropagator {
    ...
    private void markEdge(DesignerEdge e) {
        ...
        if (debug.get()) LOG.debug(String.format("Marked edge %s and updated %d Column counters.",
                                                 e.toStringPath(agraph), this.edge_cols_xref.get(e).size()));
    }
    ...
    private void unmarkEdge(DesignerEdge e) {
        ...
        if (debug.get()) LOG.debug(String.format("Unmarked edge %s and updated %d Column counters.",
                                                 e.toStringPath(agraph), this.edge_cols_xref.get(e).size()));
    }
    ...
    public void update(CatalogType catalog_obj) {
        ...
        if (catalog_obj instanceof Table) {
            ...
            if (catalog_tbl.getSystable() == false && catalog_tbl.getIsreplicated() == false && catalog_tbl.getPartitioncolumn() != null) {
                ...
                for (DesignerEdge e : all_edges) {
                    if (trace.get()) LOG.trace(String.format("Checking whether we can mark edge %s", e.toStringPath(agraph)));
                    if (edges.contains(e) == false && this.marked_edges.contains(e) == false) {
                        this.markEdge(e);
                        e_ctr++;
                    }
                } // FOR
                if (debug.get()) LOG.debug(String.format("Marked %d out of %d edges for update of %s",
                                                         e_ctr, all_edges.size(), catalog_tbl));
            }
            this.isset_tables++;
        ...
        if (debug.get()) LOG.debug("Current IsSet Items: " + this.isset);
    ...
    public void reset(CatalogType catalog_obj) {
        ...
        if (catalog_obj instanceof Table) {
            ...
            for (DesignerEdge e : this.agraph.getIncidentEdges(v)) {
                if (this.isMarked(e)) this.unmarkEdge(e);
            } // FOR
            this.isset_tables--;
    ...
    public <T extends CatalogType> Collection<T> getCandidateValues(CatalogType catalog_obj, Class<T> return_class) {
        ...
        ret.clear();
        if (debug.get()) LOG.debug("Retrieving candidate values for " + catalog_obj);
            ...
            for (DesignerEdge e : edges) {
                ...
                for (Column catalog_col : columns) {
                    ...
                    if (catalog_col.getParent().equals(catalog_tbl) && this.column_edge_ctrs.get(catalog_col).get() > 0) {
                        ...
                        if (trace.get()) LOG.trace(String.format("Adding candidate Column %s because of Edge %s",
                                                                 catalog_col.fullName(), e.toStringPath(agraph)));
                    }
                } // FOR (columns)
                total_cols += columns.size();
            } // FOR (edges)
            if (debug.get()) {
                LOG.debug(String.format("Selected %d out of %d candidate Columns using %d edges for %s",
                                        ret.size(), total_cols, edges.size(), catalog_tbl));
            }
