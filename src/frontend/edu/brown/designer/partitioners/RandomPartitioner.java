...
public class RandomPartitioner extends AbstractPartitioner {
    ...
    public PartitionPlan generate(DesignerHints hints) throws Exception {
        ...
        // Generate the list of which columns we will randomly select from for each table
        ...
        while (round >= (-1 * rounds)) { 
            ...
            for (Table catalog_tbl : info.catalog_db.getTables()) {
                ...
                // The way this works is that on each interation the value of 'round' is 
                // decreasing. This means that selecting a table replication decreases every round
                // until it's at the point where we won't select replication at all
                ...
                if (idx == size) {
                    ...
                    if (calculate_memory) total_memory += ts.tuple_size_total; 
                } else {
                    catalog_col = columns.get(idx);
                    if (calculate_memory) total_memory += ts.tuple_size_total / info.getNumPartitions();
                }
            ...
            if (total_memory <= 1.0) break;
            LOG.debug("[" + round + "] TotalMemory=" + total_memory);

            // Otherwise make it so that we are less likely to select the partitioning attribute 
            // the next go around
            round--;
        } // WHILE
