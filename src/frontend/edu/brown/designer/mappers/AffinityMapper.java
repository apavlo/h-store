...
public class AffinityMapper extends AbstractMapper {
    ...
    protected class FragmentAffinity {
        ...
        /**
         * Nasty!
         * <root0.PartitionId, root1.PartitionId> -> Histogram
         */
    ...
    public PartitionMapping generate(DesignerHints hints, PartitionPlan pplan) throws Exception {
        //
        // Unfortunately at this point we have to run this through again in order to figure
...
//        System.out.println("num_partitions="+num_partitions);
        ...
        //
        // For each transaction that is not single-sited, figure out what partitions
        ...
        for (AbstractTraceElement<?> element : this.info.workload) {
            ...
            if (xact_cost.isSinglePartitioned()) continue;
            ...
            //  (1) All of the individual queries are ss, but the total xact is ms
            ...
            //  (1) Uses data in separate partition trees (at this point we don't know whether
            //      the fragments from the different trees are on the same site)
            //  (2) Use data from the same partition tree, but based on different values
            //      of the partitioning attribute.
            //
            // So now we need to look at each query and determine what fragments they 
            // want to go at.
            //
            
            
            for (SingleSitedCostModel.QueryCacheEntry query_cost : cost_model.getQueryCacheEntries(xact)) {
//                QueryTrace query = (QueryTrace)info.workload.getTraceObject(query_cost.getQueryId());
//                assert(query != null);
...
//                    System.out.println("ROOTS: " + root0 + " <-> " + root1);
                    ...
                    for (Integer hash0 : hashes0) {
                        for (Integer hash1 : hashes1) {
                            if (hash0 != hash1 || (hash0 == hash1 && !same_root)) {
                                ...
                                if (affinity1 != null) affinity1.add(hash1, hash0);
                            }
                        } // FOR (hash1)
                    } // FOR (hash0)
                } // FOR (root1)
            ...
            xact_ctr++;
            if (xact_ctr % 100 == 0) LOG.info("Processed affinity for " + xact_ctr + " transactions...");
        } // FOR (xact)
...
//            System.out.println("[" + ctr++ + "]: " + other + " [" + other.hashCode() + "]");
//        }
//        System.out.flush();
