...
public class PlanOptimizer {
	 ...
	 */
	private static final PlanNodeType TO_IGNORE[] = { PlanNodeType.AGGREGATE,
	 ...
	 */
	public AbstractPlanNode optimize(final String sql,
			final AbstractPlanNode root) {
		// HACK
		for (String broken : BROKEN_SQL) {
			if (sql.contains(broken)) {
				if (debug.get())
					LOG.debug("Given SQL contains broken fragment '" + broken
							+ "'. Skipping...\n" + sql);
				return (null);
			}
		}
		...
		for (PlanNodeType t : TO_IGNORE) {
			if (types.contains(t)) {
				if (trace.get())
					LOG.trace(String
							.format("Tree rooted at %s contains %s. Skipping optimization...",
									root, t));
				return (null);
			}
		} // FOR
		...
		for (Class<? extends AbstractOptimization> optClass : OPTIMIZATONS) {
			...
			try {
				...
				if (p.getFirst()) {
					if (debug.get())
						LOG.debug(String.format("%s modified query plan",
								optClass.getSimpleName()));
