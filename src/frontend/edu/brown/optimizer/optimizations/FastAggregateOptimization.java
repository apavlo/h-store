...
public class FastAggregateOptimization extends AbstractOptimization {
	...
	public Pair<Boolean, AbstractPlanNode> optimize(AbstractPlanNode rootNode) {
		...
		Collection<HashAggregatePlanNode> nodes = PlanNodeUtil.getPlanNodes(
				rootNode, HashAggregatePlanNode.class);
		...
		while (iterator2.hasNext()) {
			...
			if (debug.get()) {
				LOG.debug("Type of aggregate nodes:"
						+ node1.getAggregateTypes().contains(
		...
		while (iterator.hasNext()) {
			...
			if (node1.getAggregateTypes()
				...
				if (node1.getChild(0) instanceof ReceivePlanNode) {
					ReceivePlanNode receiv_node = (ReceivePlanNode) (node1
							.getChild(0));
