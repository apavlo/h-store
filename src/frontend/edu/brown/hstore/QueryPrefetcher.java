...
public class QueryPrefetcher implements Loggable {
    ...
    public TransactionInitRequest[] generateWorkFragments(LocalTransaction ts) {
        ...
        // TODO: We don't want to serialize all this if it's only going to the
        // base partition.
        ...
        for (WorkFragment frag : fragments) {
            ...
            if (builder == null) {
                builders[site_id] = TransactionInitRequest.newBuilder().setTransactionId(ts.getTransactionId()).setProcedureId(ts.getProcedure().getId())
