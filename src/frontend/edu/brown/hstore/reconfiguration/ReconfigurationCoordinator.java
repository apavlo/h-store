...
public class ReconfigurationCoordinator implements Shutdownable {
   ...
   * @return
   */
  public boolean initReconfiguration(Integer leaderId, ReconfigurationProtocols reconfigurationProtocol, String partitionPlan) {
      ...
      try {
        ...
        if (reconfig_plan != null) {
          for (PartitionExecutor executor : local_executors) {
            ...
            executor.setReconfiguration_state(ReconfigurationState.BEGIN);
          }
        }
      } catch (Exception e) {
        LOG.error(e);
  ...
  public void prepareReconfiguration() {
    if (this.reconfigurationInProgress) {
      if (this.reconfigurationProtocol == ReconfigurationProtocols.LIVEPULL) {
        // Move the reconfiguration state to data transfer and data will be
        // pulled based on
  ...
  /**
   * Parse the partition plan and figure out the destination sites and populates
  ...
  /**
   * Invoked after receiving messages from reconfiguration leader signaling end
   * of reconfiguration
   */
