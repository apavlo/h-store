...
public abstract class AbstractTableStatisticsGenerator {
    ...
    /**
     * All child clases must implement this method that will populate the generator with TableProfiles
     */
    ...
    /**
     * Generate a DependencyGraph using the TableProfile records
     * A table will have an edge coming into it from another table if the number of tuples for it
     * is dependent on the number of tuples of the other table
     * @return
     */
    ...
    public Map<Table, TableStatistics> generate() throws Exception {
        ...
        // First generate all the TableStatistics for tables without any dependencies
        for (Entry<Table,TableProfile> e : this.table_profiles.entrySet()) {
            Table catalog_tbl = e.getKey();
            TableProfile profile = e.getValue();

            if (profile.hasDependencies()) continue;
