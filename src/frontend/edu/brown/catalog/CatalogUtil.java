        ...
        if (catalog_proc.getParameters().size() > 0 && !catalog_proc.getSystemproc()) {
            ...
            if (idx == NullProcParameter.PARAM_IDX) {
                ...
                assert(catalog_param != null) :
                    "Unexpected Null ProcParameter for " + catalog_proc.getName() + " at idx #" + idx;
            }
        }
        ...
        for (Host catalog_host : catalog_clus.getHosts()) {
            ...
            for (Site catalog_site : CatalogUtil.getSitesForHost(catalog_host)) {
                ret.add(new String[] {
                        ...
                        Integer.toString(catalog_site.getPort()),
...
//        return (CatalogUtil.getDatabase(clone_catalog));
        ...
        final Catalog clone_catalog = cloneBaseCatalog(catalog_db.getCatalog(), new ArrayList<Class<? extends CatalogType>>());
        ...
        for (Table catalog_tbl : catalog_db.getTables()) {
            ...
            assert(catalog_tbl.getColumns().size() == clone_tbl.getColumns().size()) :
                catalog_tbl.getColumns() + " != " + clone_tbl.getColumns();
        } // FOR
        ...
        for (Procedure catalog_proc : catalog_db.getProcedures()) {
            ...
            assert(catalog_proc.getParameters().size() == clone_proc.getParameters().size()) :
                catalog_proc.getParameters() + " != " + clone_proc.getParameters();
        } // FOR
    ...
    public static Catalog cloneBaseCatalog(Catalog catalog, Class<? extends CatalogType>...skip_types) {
        return (CatalogUtil.cloneBaseCatalog(catalog, Arrays.asList(skip_types)));
    }
...
//                    children.addAfter(((PlanFragment)element).getOutputdependencyids().values());
        ...
        if (!(skip_types.contains(Table.class) || skip_types.contains(Column.class) || skip_types.contains(Constraint.class))) {
    ...
    /**
     * Add a single catalog element from one catalog into the destination catalog
     * Note that this will not copy constraints for tables, since that needs to be done
     * later to ensure that any foreign key references are included properly
     * @param <T>
     * @param src_item
     * @param dest_catalog
     * @return
    ...
    public static <T extends CatalogType> T clone(T src_item, Catalog dest_catalog) {
        ...
        if (src_item instanceof Table) {
            ...
            for (Column src_col : src_tbl.getColumns()) {
                if (!(src_col instanceof MultiColumn)) CatalogUtil.clone(src_col, dest_catalog);
            } // FOR
            // Indexes
        ...
        //
            ...
            for (Column src_col : src_tbl.getColumns()) {
                ...
                for (ConstraintRef src_conref : src_col.getConstraints()) {
                    if (!src_conref.getConstraint().equals(src_cons)) continue;
...
//                    System.out.println("dest_tbl.getConstraints(): " + CatalogUtil.debug(dest_tbl.getConstraints()));
//                    System.out.println("src_confref: " + src_conref);
//                    System.out.println("src_confref.getConstraint(): " + src_conref.getConstraint());
        ...
        //
            ...
            if (src_stmt_param.getProcparameter() != null) {
                ...
                if (dest_proc_param == null) {
                    ...
                    System.out.println("dest_stmt:      " + dest_stmt_param.getParent());
                    System.out.println("src_proc_param: " + src_proc_param);
                    System.out.println("dest_proc.getParameters(): " + CatalogUtil.debug(dest_proc.getParameters()));
        ...
        for (Table src_tbl : src_db.getTables()) {
            ...
            if (dest_tbl != null) {
                for (Constraint src_cons : src_tbl.getConstraints()) {
                    // Only clone a FKEY constraint if the other table is in the catalog
                    ConstraintType cons_type = ConstraintType.get(src_cons.getType());
                    if (cons_type != ConstraintType.FOREIGN_KEY ||
                       (cons_type == ConstraintType.FOREIGN_KEY && dest_db.getTables().get(src_cons.getForeignkeytable().getName()) != null)) { 
    ...
    public static List<Site> getSitesForHost(Host catalog_host) {
        ...
        for (Site catalog_site : cluster.getSites()) {
            if (catalog_site.getHost().getName().equals(catalog_host.getName())) sites.add(catalog_site);
        } // FOR
        return (sites);
    }
...
//                System.out.println(catalog_col + ": " + CatalogUtil.getConstraints(catalog_col.getConstraints()));
    ...
    public static Set<StmtParameter> getStmtParameters(Statement catalog_stmt, ProcParameter catalog_proc_param) {
        ...
        for (StmtParameter param : catalog_stmt.getParameters()) {
            if (param.getProcparameter() != null &&
                param.getProcparameter().equals(catalog_proc_param)) found.add(param);
        } // FOR
        return (found);
    }
    ...
    public static <T extends CatalogType, U> Set<T> findAll(Iterable<T> items, String field, U value) {
        ...
        for (T catalog_item : items) {
            ...
            try {
                ...
                if (field_value.equals(value)) found.add(catalog_item);
            } catch (NullPointerException ex) {
                System.err.println(catalog_item + ": " + catalog_item.getFields());
    ...
    public static String getDisplayName(CatalogType item, boolean include_class) {
        ...
        if (item != null) {
            if (item instanceof Column || item instanceof Statement) {
                ...
                ret = item.getParent().getName() + "." + (include_class ? item : item.getName());
            } else if (item instanceof PlanFragment) {
                ret = item.getParent().getParent().getName() + "." + item.getParent().getName() + 
                      "[Fragment #" + item.getName() + "]";
            } else if (item instanceof ConstantValue) {
                ret = item.getClass().getSimpleName() + "{" + ((ConstantValue)item).getValue() + "}";
            } else {
                ret = (include_class ? item.getClass().getSimpleName() + ":" : "") + item.getName();
            }
        }
        return (ret);
    }
    ...
    /**
     * Loads a serialized catalog specification from a jar file and creates a new
     * Catalog object from it 
     * @param jar_path
     * @return
     */
    ...
    /**
     * Loads a serialized catalog specification from a text file and creates a new
     * Catalog object from it 
     * @param jar_path
     * @return
     */
    ...
    public static Column getColumn(Database catalog_db, String table_name, String column_name) {
        ...
        if (catalog_table != null) catalog_col = catalog_table.getColumns().get(column_name);
        return (catalog_col);
    }

    /**
     * Returns all the tables access/modified in all the Statements for this Procedure
     * @param catalog_proc
     * @return
     * @throws Exception
     */
    ...
    public static Set<Column> getReferencedColumns(Statement catalog_stmt) throws Exception {
        LOG.debug("Extracting table set from statement " + CatalogUtil.getDisplayName(catalog_stmt));
        ...
        new ExpressionTreeWalker() {
            ...
            protected void callback(AbstractExpression element) {
                if (element instanceof TupleValueExpression) {
                    ...
                    if (catalog_col == null) {
                        LOG.fatal("Unknown column '" + table_name + "." + column_name + "' referenced in Expression node " + element);
                        this.stop();
                        return;
                    }
    ...
    /**
     * Return all of tables referenced in the PlanNode tree, regardless if they are modified or not
     * @param catalog_db
     * @param root
     * @return
     */
    public static Set<Table> getReferencedTables(final Database catalog_db, final AbstractPlanNode root) {
        ...
        new PlanNodeTreeWalker() {
            ...
            protected void callback(AbstractPlanNode element) {
                ...
                if (table_name != null) {
                    Table catalog_tbl = catalog_db.getTables().get(table_name);
                    assert(catalog_tbl != null) : "Invalid table '" + table_name + "' extracted from " + element;
    ...
    /**
     * Returns the set of Column catalog objects modified by the given AbstractPlanNode
     * If you're looking for where we figure out what columns a PlanNode touches that is of interest to us
     * for figuring out how we will partition things, then you've come to the right place.
     * @param catalog_db
     * @param node
     * @return
     * @throws Exception
                    ...
                    switch (node.getPlanNodeType()) {
                        ...
                        case INDEXSCAN: {
                            ...
                            for (AbstractExpression exp : idx_node.getSearchKeyExpressions()) {
                                if (exp != null) columns.addAll(CatalogUtil.getReferencedColumns(catalog_db, exp));    
                            } // FOR
                        ...
                        case DELETE:
                            // I don't think we need anything here because all the columns will get get picked
    ...
    /**
     * Returns a set of all of the PlanFragments (both multi-partition and single-partition)
     * @param catalogs_stmt
     * @return
     */
    ...
    /**
     * Returns the estimated partition that a procedure will be sent to based on its input parameters
     * @param catalog_proc
     * @param params
     * @return
     * @throws Exception
     */
    public static int estimatePartition(Procedure catalog_proc, Object params[]) throws Exception {
    ...
    public static Long estimateTupleSize(Table catalog_tbl, Statement catalog_stmt, Object params[]) throws Exception {
        ...
        //
        if (CatalogUtil.CACHE_TABLE_TUPLE_SIZE.containsKey(catalog_tbl.getName())) {
            return (CatalogUtil.CACHE_TABLE_TUPLE_SIZE.get(catalog_tbl.getName()));
        }
        ...
        // MaterializePlanNode mat_node = (MaterializePlanNode)CollectionUtil.getFirst(matched_nodes);
        ...
        for (Column catalog_col : CatalogUtil.getSortedCatalogItems(catalog_tbl.getColumns(), "index")) {
            ...
            switch (type) {
                ...
                case STRING: {
                    ...
                    /*
                    AbstractExpression root_exp = mat_node.getOutputColumnExpressions().get(catalog_col.getIndex());
                    for (ParameterValueExpression value_exp : ExpressionUtil.getExpressions(root_exp, ParameterValueExpression.class)) {
                        int param_idx = value_exp.getParameterId();
                        bytes += 8 * ((String)params[param_idx]).length();
                    } // FOR
                    */
                    break;
                }
        ...
        //
        if (numerals_only) CatalogUtil.CACHE_TABLE_TUPLE_SIZE.put(catalog_tbl.getName(), bytes);
