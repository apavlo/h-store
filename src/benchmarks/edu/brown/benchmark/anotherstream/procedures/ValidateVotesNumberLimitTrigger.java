...
public class ValidateVotesNumberLimitTrigger extends VoltTrigger {
    ...
    public final SQLStmt insertS1Stmt = 
        new SQLStmt("INSERT INTO S2 (vote_id, phone_number, state, contestant_number, created) SELECT S1.* FROM S1, votes_by_phone_number, configuration WHERE (S1.phone_number=votes_by_phone_number.phone_number) and (configuration.name='threshold') and (votes_by_phone_number.num_votes<configuration.value);");
