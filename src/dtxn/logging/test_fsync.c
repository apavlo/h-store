// NOTE: Taken from PostgreSQL, released under a BSD license:
// http://www.postgresql.org/about/licence
//
// Source:
// http://anoncvs.postgresql.org/cvsweb.cgi/pgsql/src/tools/fsync/test_fsync.c
...
main(int argc, char *argv[])
{
    ...
    buf = full_buf;
    if (((intptr_t) full_buf & (PAGE_ALIGNMENT-1)) != 0) {
        buf = (void*) ((intptr_t) full_buf & ~(PAGE_ALIGNMENT-1)) + PAGE_ALIGNMENT;
    }
    printf("%p %p\n", full_buf, buf);
    assert(((intptr_t) buf & (PAGE_ALIGNMENT-1)) == 0);
    assert(XLOG_SEG_SIZE > WRITE_SIZE);
