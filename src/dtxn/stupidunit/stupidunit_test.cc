...
bool testFail() {
    ...
    if (!strings::endsWith(test.stupidunitError(0), FailTestE1)) return false;
    if (!strings::endsWith(test.stupidunitError(1), FailTestE2)) return false;
...
bool validateBinaryOperation(const Test& test, const char* op_string, bool test1_success, bool test2_success, bool test3_success) {
    ...
    int error = 0;
    if (!test1_success) {
        if (!strings::endsWith(test.stupidunitError(error), message1)) return false;
        error += 1;
    }
    if (!test2_success) {
        if (!strings::endsWith(test.stupidunitError(error), message2)) return false;
        error += 1;
    }
    if (!test3_success) {
        if (!strings::endsWith(test.stupidunitError(error), message3)) return false;
        error += 1;
    }
    return true;
}
...
bool testExpectTrue() {
    ...
    if (test.testSuccess()) return false;
    if (test.stupidunitNumErrors() != 2) return false;
    if (!strings::endsWith(test.stupidunitError(0), "is false")) return false;
    if (!strings::endsWith(test.stupidunitError(1), "is true")) return false;
    return true;
}
...
bool testAssert() {
    ...
    if (!strings::endsWith(test.stupidunitError(0), "3 == 5")) return false;
...
TEST(ExpectDeath, FailsToDie) {
    ...
    EXPECT_TRUE(strings::endsWith(test.stupidunitError(0), "did not die"));
}
...
TEST(ChTempDir, Simple) {
    string tempdir_name;
    {
        ...
        int fd = open(name.c_str(), O_CREAT | O_WRONLY | O_EXCL, 0600);
        EXPECT_NE(0, fd);
        int status = close(fd);
        EXPECT_EQ(0, status);
...
TEST_F(StupidUnitOutputTest, OutputSuccess) {
    ...
    string out = strings::readFile(out_name_.c_str());
...
TEST_F(StupidUnitOutputTest, OutputFailure) {
    ...
    EXPECT_TRUE(strings::endsWith(out, "\"}]\n"));
}
