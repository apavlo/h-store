...
-- alias fun
-- ticket 231
SELECT (_variable + 5) AS NUMSUM FROM @from_table WHERE NUMSUM > @cmp_type
SELECT (_variable + 5) AS NUMSUM FROM @from_table ORDER BY NUMSUM
...
-- ticket 232
SELECT NUM FROM @from_tables ORDER BY NUM GROUP BY NUM
