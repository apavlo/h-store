...
import edu.brown.benchmark.locality.procedures.GetLocal;
...
//        System.out.println("Inline plan node size: " + scan_node.getInlinePlanNodes().size());
...
//    }
    ...
    /**
     * testProjectionPushdown2 
     * For the query select A.value from A, B where B.id = A.id, 
     * Make sure projection on A doesn't throw away A.id before it can
     * be used for the join.
     */
    public void testProjectionPushdown2() throws Exception {
        Procedure catalog_proc = this.getProcedure(GetLocal.class);
        Statement catalog_stmt = this.getStatement(catalog_proc, "testLocal");
