...
public class ABCClient extends BenchmarkComponent {
	...
	// --------------------------------------------------------------------
	public interface ABCParamGenerator {
		public Object[] generate(AbstractRandomGenerator rng,
				ExecutionType mtype, Catalog catalog,
				Map<String, Long> table_sizes);
	}
	...
	public enum Transaction {
		GetTableCounts(ABCConstants.FREQUENCY_GET, new ABCParamGenerator() {
			@Override
			public Object[] generate(AbstractRandomGenerator rng,
					ExecutionType mtype, Catalog catalog,
					Map<String, Long> table_sizes) {
				int aid = rng.nextInt(table_sizes.get(
						ABCConstants.TABLENAME_TABLEA).intValue());
				Object params[] = new Object[] { getDataId(
						Long.valueOf(String.valueOf(aid)), rng, mtype, catalog,
						table_sizes) };
				return (params);
		...
		private Transaction(int weight, ABCParamGenerator generator) {
			this.weight = weight;
		...
		public Object[] params(AbstractRandomGenerator rng,
				ExecutionType mtype, Catalog catalog,
				Map<String, Long> table_sizes) {
		...
		private final ABCParamGenerator generator;
	 ...
	 * @return
	 */
	protected static long getDataId(long a_id, AbstractRandomGenerator rng,
			ExecutionType type, Catalog catalog, Map<String, Long> table_sizes) {
		long a_id2 = -1;
		Cluster catalog_clus = CatalogUtil.getCluster(catalog);
		long temp = ABCConstants.TABLESIZE_TABLEA
		...
		switch (type) {
		case SAME_PARTITION: {
			int partition_num = TheHashinator.hashToPartition(a_id,
					catalog_clus.getNum_partitions());
			System.out.println("Total number of partitions: "
					+ catalog_clus.getNum_partitions());
			a_id2 = random_int * catalog_clus.getNum_partitions()
					+ partition_num;
			break;
		}
		case SAME_SITE: {
			int partition_num = TheHashinator.hashToPartition(a_id,
					catalog_clus.getNum_partitions());
			Site site = CatalogUtil.getPartitionById(catalog, partition_num)
					.getParent();
			int num_sites_per_host = CatalogUtil.getSitesPerHost(site)
					.get(site.getHost()).size();
			int num_partitions_per_site = site.getPartitions().size();
			double a_id_site_num = Math.floor((double) partition_num
					/ (double) num_partitions_per_site);
			double a_id_host_num = Math.floor((double) a_id_site_num
					/ (double) num_sites_per_host);
			...
			int lowerbound = (int) a_id_host_num * num_sites_per_host
					* num_partitions_per_site + (int) a_id_site_num
					* num_partitions_per_site;
			int upperbound = (int) a_id_host_num * num_sites_per_host
					* num_partitions_per_site + (int) a_id_site_num
					* num_partitions_per_site + (num_partitions_per_site - 1);
			int a_id2_partition_num = rng.numberExcluding(lowerbound,
					upperbound, partition_num);
		...
		case SAME_HOST: {
			...
			int upperboundsite = (int) a_id_host_num * num_sites_per_host
					+ (num_sites_per_host - 1);
			int new_site = rng.numberExcluding(lowerboundsite, upperboundsite,
					(int) a_id_site_num);
			int lowerbound = new_site * num_partitions_per_site;
			int upperbound = new_site * num_partitions_per_site
					+ (num_partitions_per_site - 1);
		...
		case REMOTE_HOST: {
			...
			if (total_number_of_hosts > 1) {
				new_host = rng.numberExcluding(0, total_number_of_hosts - 1,
						(int) a_id_host_num);
			}
			...
			// host and random sites)
			// and pick partition randomly from this range
			int lowerbound = new_host * num_sites_per_host
					* num_partitions_per_site + new_site
					* num_partitions_per_site;
			int upperbound = new_host * num_sites_per_host
					* num_partitions_per_site + new_site
					* num_partitions_per_site + (num_partitions_per_site - 1);
			int a_id2_partition_num = rng.number(lowerbound, upperbound);
			a_id2 = random_int * catalog_clus.getNum_partitions()
		...
		case RANDOM: {
			a_id2 = rng.nextInt(table_sizes.get(ABCConstants.TABLENAME_TABLEA)
					.intValue());
			break;
		}
		default:
	...
	public static void main(String args[]) {
		edu.brown.benchmark.BenchmarkComponent.main(ABCClient.class, args,
				false);
	}
	...
	public ABCClient(String[] args) {
		...
		// Number of Records Per Table
		this.table_sizes.put(
				ABCConstants.TABLENAME_TABLEA,
				Math.round(ABCConstants.TABLESIZE_TABLEA
						* hstore_conf.client.scalefactor));
		this.table_sizes.put(
				ABCConstants.TABLENAME_TABLEB,
				Math.round(ABCConstants.TABLESIZE_TABLEB
						* hstore_conf.client.scalefactor));
		for (String tableName : ABCConstants.TABLENAMES) {
			assert (this.table_sizes.containsKey(tableName)) : "Missing table size entry for "
					+ tableName;
		} // FOR
	}
	...
	protected boolean runOnce() throws IOException {
		...
		boolean ret = this.getClientHandle().callProcedure(
				new ABCCallback(txn_type), txn_type.name(), params);
		return (ret);
	}
	 ...
	 */
	protected class ABCCallback implements ProcedureCallback {
		private final Transaction txn;

		public ABCCallback(Transaction txn) {
			super();
			this.txn = txn;
		}
