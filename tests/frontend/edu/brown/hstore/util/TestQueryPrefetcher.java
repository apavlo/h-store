...
//            } // FOR
    ...
    public void testGenerateWorkFragments() throws Exception {
    	...
    	TransactionInitRequest[] requests = prefetcher.generateWorkFragments(this.ts);
    	assertEquals(num_sites, requests.length);
    	
        // The TransactionInitRequest for the local partition will be the default, the next partition will have a regular WorkFragment, and the rest will be null.
    	System.err.println(StringUtil.join("\n\n", requests));
    	/*TransactionInitRequest default_request = TransactionInitRequest.newBuilder()
                .setTransactionId(this.ts.getTransactionId())
                .setProcedureId(this.ts.getProcedure().getId())
                .addAllPartitions(this.ts.getPredictTouchedPartitions())
                .build();*/
