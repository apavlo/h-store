 ...
 * @author jdebrabant
 */
public class TestAntiCacheNVMEvicted extends BaseTestCase {
    ...
    public void setUp() throws Exception {
        ...
        this.hstore_conf.site.anticache_dbtype = "ALLOCATORNVM";
    ...
    public void testStats() throws Exception {
        ...
        for (int i = 0; i < statsFields.length; i++) {
            // ACTIVE
            assertEquals(statsFields[i], origStats[0].getLong(statsFields[i]), newStats[0].getLong(statsFields[i]));
            
            // GLOBAL WRITTEN
            assertEquals(writtenFields[i], origStats[0].getLong(writtenFields[i]), newStats[0].getLong(writtenFields[i]));
            
            // GLOBAL READ
            assertEquals(readFields[i], 0, newStats[0].getLong(readFields[i]));
        } // FOR
