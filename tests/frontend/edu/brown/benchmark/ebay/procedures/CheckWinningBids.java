...
public class CheckWinningBids extends VoltProcedure{
	...
	public final SQLStmt select_due_items = new SQLStmt(
		"SELECT i_id, i_u_id " + 
		"FROM " + EbayConstants.TABLENAME_ITEM + " " + 
	...
	*/
	...
	public final SQLStmt select_max_bid = new SQLStmt(
			"SELECT imb_ib_id " + 
			"FROM " + EbayConstants.TABLENAME_ITEM_MAX_BID + " " +  
			"WHERE imb_i_id = ? AND imb_u_id = ? "
	);
	
	public final SQLStmt select_item_bid = new SQLStmt(
    		"SELECT ib_buyer_id " + 
    		"FROM " + EbayConstants.TABLENAME_ITEM_BID + " " + 
    ...
    */
    ...
    /*
     * "LEFT OUTER JOIN " + EbayConstants.TABLENAME_ITEM_BID + " ON i_id = ib_i_id AND i_u_id = ib_u_id " +
        "LEFT OUTER JOIN " + EbayConstants.TABLENAME_ITEM_MAX_BID + " ON ib_id = imb_ib_id AND ib_u_id = imb_u_id " + 
        "WHERE i_start_date >= ? AND i_end_date <= ? AND i_u_id >= ? AND i_u_id < ?"
     */
