...
public class NewPurchase extends VoltProcedure{
    ...
    public VoltTable run(long bid_id, long item_id, long seller_id, long buyer_id) throws VoltAbortException {
    	...
    	if (results[0].getRowCount() == 1) {
    		...
    		if(buyer_id != ib_buyer_id){
    	    	throw new VoltAbortException("User " + buyer_id + " did not win the bit " + bid_id);
    		}
    	} else {
    	...
    	voltQueueSQL(select_max_purchase_id, bid_id, item_id, seller_id);
    	results = voltExecuteSQL();
    	assert(results.length == 1);
        ...
        voltQueueSQL(insert_purchase, ip_id, bid_id, item_id, seller_id, new TimestampType());
        voltExecuteSQL();
        ...
        ret.addRow(new Object[]{ip_id, bid_id, item_id, seller_id, buyer_id});
