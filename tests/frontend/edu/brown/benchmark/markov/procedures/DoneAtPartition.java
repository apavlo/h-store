...
/**
 * DoneProcedure
 * This procedure has a series of multi partition writes and reads
 * followed by a single single-partition write. The MarkovGraph should reflect this.
 */
public class DoneAtPartition extends VoltProcedure {
    public final SQLStmt fromC = new SQLStmt(
            "SELECT C_ID, C_A_ID FROM " + MarkovConstants.TABLENAME_TABLEC + " WHERE C_IATTR01 = ?");
    public final SQLStmt updateB = new SQLStmt(
            "UPDATE " + MarkovConstants.TABLENAME_TABLEB + " SET B_IATTR01 = B_IATTR01 + ? WHERE B_A_ID = ?");
    public final SQLStmt selectB = new SQLStmt(
            "SELECT B_IATTR01, B_ID, B_A_ID  " +
            "  FROM " + MarkovConstants.TABLENAME_TABLEB + 
            " WHERE B_A_ID = ? " +
            " ORDER BY B_IATTR01 DESC LIMIT 1");
    public final SQLStmt updateA = new SQLStmt(
            "UPDATE " + MarkovConstants.TABLENAME_TABLEA +
            "   SET A_IATTR01 = ?, " + // B_ID
    ...
    public VoltTable[] run(long a_id, long value) {
        ...
        while (intermediate_results[0].advanceRow()) {
            ...
            // however, this update is not necessarily on the same partition as our current
            // a_id, so it will go to other partitions to conduct the update
            c_a_ids.add(c_a_id);
        } // WHILE
        voltExecuteSQL();
        
        // Because we can't do the distributed aggregate using IN, we have to manually
        // find the max for ourselves
        long maxBValue = 0;
        long maxID = 0;
        for (Long c_a_id : c_a_ids) {
            voltQueueSQL(selectB, c_a_id);
            //(MP) We are selecting from B based on a bunch of different c_a_ids we retrieved
            // from our first call. As a result, this is going to be multi-partition most of the time.
